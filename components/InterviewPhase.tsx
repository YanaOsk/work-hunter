...
export default function InterviewPhase({ userProfile, onComplete, onBack, initialMessages, initialConvId, initialReadyToSearch }: Props) {
  ...
  useEffect(() => {
    ...
    if (firstMessage) {
      ...
      // Sanitize all strings before JSON.stringify.
      // Garbled PDF text can contain lone surrogates that make JSON.stringify throw
      // in the browser (Chrome V8), silently killing the fetch → fallbackGreeting.
      // Node.js serializes them fine, but the browser can't re-serialize after parsing.
      const stripSurrogates = (_k: string, v: unknown) =>
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      const stripSurrogates2 = (_k: string, v: unknown) =>
        typeof v === "string" ? v.replace(/[\uD800-\uDFFF]/g, "?") : v;
