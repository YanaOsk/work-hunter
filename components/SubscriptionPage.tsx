...
const PLAN_META: Record<string, { nameHe: string; nameEn: string; cls: string; iconCls: string }> = {
  ...
  "one-time": { nameHe: "המסע המלא",   nameEn: "Full Journey", cls: "border-purple-500/50 bg-purple-500/10", iconCls: "text-purple-300" },
...
export default function SubscriptionPage() {
  ...
  // Cancel = remove card so subscription won't auto-renew, but stays active until expiry
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-purple-950/30 to-slate-900">
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        {!loading && sub && planMeta && (
          <div className={`rounded-3xl border overflow-hidden ${planMeta.cls}`}>

            {/* Plan header */}
            <div className="p-6 flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center ${planMeta.iconCls}`}>
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                </div>
                <div>
                  <p className={`text-xl font-bold ${planMeta.iconCls}`}>{he ? planMeta.nameHe : planMeta.nameEn}</p>
              ...
              {/* Expiry / renewal info — right side */}
              <div className="text-right shrink-0">
                {sub.isLifetime ? (
                  ...
                  <div>
                    {sub.savedCard ? (
                      <>
                        <p className="text-white/40 text-xs mb-0.5">{he ? "מתחדש אוטומטית ב" : "Auto-renews on"}</p>
                        <p className="text-white font-semibold text-sm">{formatDate(sub.expiryDate, he)}</p>
                      </>
                    ) : (
                      <>
                        <p className="text-white/40 text-xs mb-0.5">{he ? "פעיל עד" : "Active until"}</p>
                        <p className="text-amber-300 font-semibold text-sm">{formatDate(sub.expiryDate, he)}</p>
                      </>
                    )}
                  </div>
                ) : null}
