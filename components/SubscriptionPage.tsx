  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-12">
        ...
        {!loading && (
          <div className="mt-10">
            <p className="text-white/40 text-xs font-semibold uppercase tracking-widest mb-5 text-center">
              {he ? "המסלולים הזמינים" : "Available plans"}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {(["weekly", "quarterly", "lifetime"] as const).map((planId) => {
                ...
                return (
                  <div
                    key={planId}
                    className={`relative rounded-2xl border ${meta.borderCls} bg-white/3 overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:shadow-xl`}
                  >
                    {/* Animated glow */}
                    <div className={`absolute inset-0 bg-gradient-to-br ${meta.glowFrom} ${meta.glowTo} opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none`} />
                    {/* Pulse ring for current plan */}
                    {isCurrent && (
                      <div className={`absolute inset-0 rounded-2xl border-2 ${meta.borderCls} animate-pulse pointer-events-none`} />
                    )}
                    <div className="relative p-5 flex flex-col gap-3">
                      {/* Badge */}
                      {isCurrent && (
                        <span className={`self-start text-[10px] font-bold px-2 py-0.5 rounded-full border ${meta.badgeCls}`}>
                          {he ? "המנוי שלך" : "Current"}
                        </span>
                      )}
                      {/* Name */}
                      <p className={`text-lg font-black ${meta.iconCls}`}>
                        {he ? meta.nameHe : meta.nameEn}
                      </p>
                      {/* Price */}
                      <div className="flex items-end gap-1.5">
                        <span className={`text-3xl font-black ${meta.priceCls}`}>{plan.displayPrice}</span>
                        {plan.per && <span className="text-white/35 text-xs mb-1">{plan.per}</span>}
                      </div>
                      {/* Features */}
                      <ul className="space-y-1.5 mt-1">
                        {plan.featuresHe.map((f, i) => (
                          <li key={i} className="flex items-start gap-2 text-white/60 text-xs leading-snug">
                            <svg className={`w-3.5 h-3.5 shrink-0 mt-0.5 ${meta.checkCls}`} fill="currentColor" viewBox="0 0 20 20">
                      ...
                      {!isCurrent && (
                        <button
                          ...
                          className={`mt-2 w-full py-2 rounded-xl text-xs font-semibold border ${meta.borderCls} ${meta.iconCls} hover:bg-white/10 transition`}
                        >
                          {he ? "בחירה" : "Choose"}
                        </button>
                      )}
