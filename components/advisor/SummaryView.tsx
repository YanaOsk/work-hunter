...
interface Props {
  ...
  onLaunchCv?: () => void;
  onLaunchLinkedin?: () => void;
}

export default function SummaryView({ advisorState, onBack, onOpenInterview, onExit, onUpdate, onLaunchCv, onLaunchLinkedin }: Props) {
  ...
  return (
    <div style={{ background: "var(--background)" }} className="min-h-screen p-4 md:p-6">
      <div className="max-w-4xl mx-auto" ref={contentRef}>
        ...
        {/* Section 3: CV Review */}
        {!cvReview && !advisorState.cvSkipped && onLaunchCv && (
            ...
            <div className="flex items-center gap-4 py-2">
              ...
              <div className="flex-1">
                <p className="text-white/70 text-sm leading-relaxed">
                  {lang === "he"
                    ? "קבלו ציון, נקודות לשיפור וסיכום מחודש לקורות החיים שלכם — מותאם לתפקיד שבחרתם."
                    : "Get a score, improvement tips, and a rewritten summary tailored to your target role."}
                </p>
              </div>
              <button
                onClick={onLaunchCv}
                className="flex-shrink-0 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold px-4 py-2 rounded-xl transition shadow-lg shadow-blue-500/20"
              >
                {lang === "he" ? "שפר קו\"ח" : "Review CV"}
              </button>
            </div>
          </Section>
        )}
